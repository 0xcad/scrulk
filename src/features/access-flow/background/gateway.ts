...
import { reduceAccessFlow, shouldInterruptAllowance } from "../transitions";

const accessBase = () => browser.runtime.getURL(EXTENSION_PAGES.gateway);
const surveyBase = () => browser.runtime.getURL(EXTENSION_PAGES.survey);
...
export async function syncTrackedTabPresence(
  removedTabId?: number,
): Promise<void> {
  ...
  const hasTracked = tabs.some((tab) => {
    if (tab.id === removedTabId) return false;
    const host = hostnameOf(lifecycleUrl(tab));
    return host !== null && isTracked(host, settings.trackedSites);
  });
  if (shouldInterruptAllowance(state, hasTracked, Date.now())) {
    await setDayState(reduceAccessFlow(state, { type: "allowanceInterrupted" }));
  }
}
